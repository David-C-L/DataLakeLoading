#include "DADSParquetReaderEngine.hpp"
#include <DADS.hpp>
#include <Engine.hpp>
...
using dads::expressions::ExpressionSpanArgument;
using dads::expressions::ExpressionSpanArguments;

using dads::Expression;

constexpr static int64_t FETCH_PADDING = 0;
constexpr static int64_t FETCH_ALIGNMENT = 4096;
constexpr static int64_t MAX_RANGES = 2;

namespace dads::engines::ParquetReader {
namespace utilities {
...
dads::Expression applyEngine(Expression &&e, EvalFunction eval) {
  ...
  int64_t getFileLength(const std::string &url, EvalFunction loader) {
    ...
    args.push_back(url);
    auto getFileLenExpr = dads::ComplexExpression("GetFileLength"_, {}, std::move(args), {});

    return get<int64_t>(applyEngine(std::move(getFileLenExpr), loader));
  }


dads::Expression createFetchExpression(const std::string &url,
                              std::vector<int64_t> &bounds) {
  ExpressionArguments args;
  args.push_back(std::move("List"_(dads::Span<int64_t>(bounds))));
  args.push_back(url);
  args.push_back((int64_t)FETCH_PADDING);
  args.push_back((int64_t)FETCH_ALIGNMENT);
  args.push_back((int64_t)MAX_RANGES);

  return dads::ComplexExpression("Fetch"_, {}, std::move(args), {});
}

  
dads::Span<int8_t> getByteSequence(dads::ComplexExpression &&expression) {
  ...
  if (!std::holds_alternative<dads::Span<int8_t>>(typedSpan)) {
    throw std::runtime_error(
        "unsupported span type: ByteSequence span is not an int8_t span");
  }
  auto &byteSpan = std::get<dads::Span<int8_t>>(typedSpan);
  return std::move(byteSpan);
}

dads::Span<int8_t> loadBoundsIntoSpan(const std::string &url,
                           std::vector<int64_t> &bounds,
                           EvalFunction &loader) {
  auto fetchExpr = createFetchExpression(url, bounds);
  auto byteSeqExpr = std::get<dads::ComplexExpression>(
      applyEngine(std::move(fetchExpr), loader));
  return std::move(getByteSequence(std::move(byteSeqExpr)));
}

  std::vector<int64_t> getFooterBounds(dads::Span<int8_t> &byteSpan, int64_t fileLength) {
  ...
  std::vector<int64_t> getColumnWithMinMaxBounds(std::unique_ptr<parquet::arrow::FileReader> &parquetReader, boost::dynamic_bitset<uint8_t> &colChunkIDs, dads::Span<int8_t> &byteSpan, EvalFunction &loader, const std::string &url) {
    std::vector<int64_t> resultBounds;
  ...
  void readColumnChunkIntoSpans(std::unique_ptr<parquet::arrow::FileReader> &parquetReader, int64_t rowGroupI, int64_t colChunkI, ExpressionSpanArguments &resSpans) {
	...
	if constexpr(std::is_convertible_v<decltype(columnArray), arrow::StringArray const&>) {
	  ...
	  auto stringSpan = dads::Span<std::string>(std::move(std::vector(std::move(stringVec))));
	  ...
	  if constexpr(std::is_constructible_v<expressions::ExpressionSpanArgument,
		       dads::Span<ElementType>> &&
		       std::is_constructible_v<dads::Span<ElementType>, ElementType*, int,
		       std::function<void(void)>>) {
	    resSpans.emplace_back(dads::Span<ElementType>(
...
#ifdef DEBUG
    std::cout << "DESERIALISING INTO DADS TABLE" << std::endl;
#endif
  ...
  dads::Expression getColumnsFromBitsetInTableExpression(std::unique_ptr<parquet::arrow::FileReader> &parquetReader, std::vector<std::string> &colNames, boost::dynamic_bitset<uint8_t> &colChunkIDs) {
    ...
    for (auto& [colName, colSpans] : colToSpansMap) {
      ...
      auto listExpr = dads::ComplexExpression("List"_, {}, {}, std::move(colSpans));
      auto colHead = dads::Symbol(colName);
      colArgs.emplace_back(std::move(listExpr));
      auto colExpr = dads::ComplexExpression(std::move(colHead), {}, std::move(colArgs), {});
      tableArgs.emplace_back(std::move(colExpr));
    }
    auto tableExpr = dads::ComplexExpression("Table"_, {}, std::move(tableArgs), {});
    return std::move(tableExpr);
  }

  std::vector<std::pair<double_t, double_t>> getMinMaxList(dads::ComplexExpression &&expression) {
    std::vector<std::pair<double_t, double_t>> minMaxList;
    auto [head, unused_, dynamics, spans] = std::move(expression).decompose();
    if (head != "List"_ || dynamics.size() % 2 != 0) {
      // empty assumes get whole column
      return minMaxList;
    }
  ...
  std::unordered_map<std::string, std::vector<std::pair<double_t, double_t>>> getColumnsWithMinMax(dads::ComplexExpression &&expression) {
    ...
    std::for_each(std::make_move_iterator(dynamics.begin()),
		    ...
		    auto [colHead, unused2_, colDynamics, colSpans] = std::move(get<dads::ComplexExpression>(std::move(arg))).decompose();
#ifdef DEBUG
                    std::cout << "SELECTED: " << colHead << std::endl;
#endif
		    auto minMaxList = getMinMaxList(get<dads::ComplexExpression>(std::move(colDynamics[0])));
...
dads::Expression Engine::evaluate(Expression &&e) {
  return std::visit(
      dads::utilities::overload(
	      ...
	      auto columns = getColumnsWithMinMax(std::move(get<dads::ComplexExpression>(std::move(dynamics[2]))));
		...
		colNames.push_back(colName);
	      ...
	      #ifdef DEBUG
	      std::cout << "DERIVED ARGUMENTS TO GETCOLUMNSFROMPARQUET" << std::endl;
	      #endif
	      
	      int64_t fileLength = getFileLength(url, loader);
	      dads::Span<int8_t> byteSpan;
	      std::vector<int64_t> metadataBounds = {0, 4, fileLength-8, fileLength-1};
	      byteSpan = loadBoundsIntoSpan(url, metadataBounds, loader);

	      #ifdef DEBUG
	      std::cout << "LOADED START AND END BOUNDS (fileLength = " << fileLength << " )" << std::endl;
	      #endif
	      ...
	      #ifdef DEBUG
	      std::cout << "OPENED BUFFER AS PARQUET" << std::endl;
	      printParquetSchema(parquetReader);
	      #endif
		...
		currColChunksMap[url] = std::move(colChunksToLoadBitset);
	      ...
	      // auto columnBounds = getColumnWithMinMaxBounds(parquetReader, currColChunksToLoadBitset);
	      // byteSpan = loadBoundsIntoSpan(url, columnBounds, loader);
...
static auto &enginePtr(bool initialise = true) {
  static auto engine = std::unique_ptr<dads::engines::ParquetReader::Engine>();
  if (!engine && initialise) {
    engine.reset(new dads::engines::ParquetReader::Engine());
  }
  return engine;
}
