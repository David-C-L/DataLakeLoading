...
def create_tpch_queries_graph(json_path, csv_size_path, csv_time_path, plot_pdf):
    ...
    config_order = ['Data Vaults',
                    'Load-Evaluate']  # DV first, then Load-Evaluate, ensures Load-Evaluate is on the right
    ...
    avg_data = grouped.apply(lambda x: x['TotalTransferred'].sum() / x['Iteration'].max()).reset_index(
        name='AvgTransferred')
    ...
    avg_time = time_grouped.apply(lambda x: x['TotalTransferTime'].sum() / x['Iteration'].max()).reset_index(
    ...
    for i, (ax, query) in enumerate(zip(axes.flatten(), queries)):
        ...
        pivot_table = subset.pivot_table(index='Scale Factor', columns='Configuration', values='Real Time (s)',
                                         aggfunc='mean')
        ...
        for idx, row in csv_subset.iterrows():
            ...
            ax2.hlines(total_transfer, bar_x - bar_width / 2, bar_x + bar_width / 2, colors=line_color, linestyles='-',
                       linewidth=2)

        # # Plot overall average lines
        # ax.axhline(y=avg_execution_time, color='darkorange', linestyle='--', linewidth=2)
        # ax2.axhline(y=avg_data_transferred, color='orange', linestyle='--', linewidth=2)
...
def create_tpch_parquet_graph(json_path, output_pdf):
    # Load JSON data
    with open(json_path, 'r') as file:
        data = json.load(file)

    # Define the queries and scale factors
    ...
    max_avg_transferred = 0
    max_avg_time = 0

    # Loop through the data to find the maximum transferred data (bytes)
    for i, query in enumerate(queries):
        query_data = data['queries'][i]
        for scale_factor in scale_factors:
            max_avg_transferred = max(max_avg_transferred,
                                      query_data[str(scale_factor)]['parquet_bytes'] / (1024 * 1024),
                                      query_data[str(scale_factor)]['wisent_bytes'] / (1024 * 1024))
            max_avg_time = max(max_avg_time,
                               query_data[str(scale_factor)]['parquet_avg_milliseconds'] / 1000,
                               query_data[str(scale_factor)]['wisent_avg_milliseconds'] / 1000)

    # Calculate transfer_y_lim based on the max average transferred data
    transfer_y_lim = 10 ** (ceil(log(max_avg_transferred, 10)) + 1)
    # Calculate transfer_y_lim based on the max average transferred data
    time_y_lim = 10 ** ceil(log(max_avg_time, 10))
    ...
    for i, query in enumerate(queries):
        ...
        query_data = data["queries"][i]  # Each element corresponds to a query
        ...
        # Loop over scale factors and gather data
        for scale_factor in scale_factors:
            parquet_times.append(query_data[str(scale_factor)]['parquet_avg_milliseconds'] / 1000)  # Convert to seconds
            wisent_times.append(query_data[str(scale_factor)]['wisent_avg_milliseconds'] / 1000)  # Convert to seconds
            parquet_bytes.append(query_data[str(scale_factor)]['parquet_bytes'] / (1024 * 1024))  # Convert to MB
            wisent_bytes.append(query_data[str(scale_factor)]['wisent_bytes'] / (1024 * 1024))  # Convert to MB

        # Plot the bars for parquet and wisent times
        bar_width = 0.35
        x = np.arange(len(scale_factors))
        ax.bar(x - bar_width / 2, parquet_times, bar_width, label='Parquet (s)', color=palette[1], log=True)
        ax.bar(x + bar_width / 2, wisent_times, bar_width, label='Wisent (s)', color=palette[0], log=True)

        # Add the horizontal lines representing data transfer in MB
        for j in range(len(scale_factors)):
            ax.hlines(parquet_bytes[j], x[j] - bar_width / 2 - 0.15, x[j] - bar_width / 2 + 0.15, color='b',
                      linewidth=2)
            ax.hlines(wisent_bytes[j], x[j] + bar_width / 2 - 0.15, x[j] + bar_width / 2 + 0.15, color='r', linewidth=2)

        # Set titles, labels, and ticks
        ax.set_title(f"{query}", fontsize=14)
        ax.set_xticks(x)
        ax.set_xticklabels(scale_factors)
        ax.set_xlabel('')
        ax.set_yscale('log', base=10)
        ax.yaxis.set_major_formatter(FuncFormatter(log_tick_formatter))
        ax.set_ylim(1, time_y_lim)
        if i == 0:
        ...
        # Create a second y-axis for data transferred in MB
        ax2 = ax.twinx()
        ax2.set_yscale('log', base=10)
        ax2.set_ylim(1, transfer_y_lim)
        ax2.yaxis.set_major_formatter(FuncFormatter(log_tick_formatter))
        ax2.grid(False)
        if i == len(queries) - 1:
            ax2.set_ylabel('Data Transferred (MB)', fontsize=14)
            ax2.tick_params(labelright=True, labelsize=12)
        else:
    ...
    # Custom legends for execution time and data transfer
    time_legend_handles = [
        mlines.Line2D([], [], color=palette[1], marker='s', linestyle='None', markersize=10, label='Runtime (Parquet)'),
        mlines.Line2D([], [], color=palette[0], marker='s', linestyle='None', markersize=10, label='Runtime (Wisent)')
    ]

    transfer_legend_handles = [
        mlines.Line2D([], [], color='b', linestyle='-', linewidth=2, label='Data Transferred (Parquet)'),
        mlines.Line2D([], [], color='r', linestyle='-', linewidth=2, label='Data Transferred (Wisent)')
    ]
    ...
    # Set the layout and save the plot to a PDF
    plt.tight_layout(rect=(0.0, 0.0, 1.0, 0.95))
...
def create_dads_framework_overhead_graph(csv_path, plot_pdf_path):
    # Read the CSV file into a DataFrame
    df = pd.read_csv(csv_path, sep="\t")

    # Calculate the total CPU Time for the "DADS" module
    dads_time = df[(df['Module'] == 'libDADS.so') | (df['Module'] == 'libDADSLazyLoadingCoordinatorEngine.so')][
        'CPU Time'].sum()

    # Calculate the total CPU Time for all other modules
    other_time = df[(df['Module'] != 'libDADS.so') & (df['Module'] != 'libDADSLazyLoadingCoordinatorEngine.so')][
        'CPU Time'].sum()

    categories = ['DADS', 'Others']
    values = [dads_time, other_time]
    ...
    ax.set_title('CPU Time Comparison: DADS vs Others', fontsize=16, fontweight='bold')
    ax.legend()
...
def plot_dads_framework_overhead_manual_vtune_graph(csv_path, pdf_path):
    # Read the CSV file
    data = pd.read_csv(csv_path)

    # Calculate the time excluding DADS
    data['Kernel'] = data['Total'] - data['DADS']
    ...
    for i, ax in enumerate(axes):
        ...
        dads_time = data['DADS'][i]
        kernel_time = data['Kernel'][i]

        # Scale the times
        scaling_factor = total_time / sf1_total
        dads_time_scaled = dads_time / scaling_factor
        kernel_time_scaled = kernel_time / scaling_factor

        # Plot DADS time
        ax.barh(0, dads_time_scaled, height=bar_height, color=pastel_orange, label='DADS Time' if i == 0 else "")
        # Plot Kernel time
        ax.barh(0, kernel_time_scaled, height=bar_height, left=dads_time_scaled, color=darker_pastel_blue,
        ...
        # Add the DADS time in a white box with a thin black border
        ax.text(dads_time_scaled + 0.1, 0, f'{dads_time:.0f}s', va='center', ha='left', color='black',
                fontsize=fontsize,
                bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'))
        # Add the total time in a white box with a thin black border
        ax.text(dads_time_scaled + kernel_time_scaled + 0.1, 0, f'{total_time:.0f}s', va='center', ha='left',
                color='black', fontsize=fontsize,
                bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'))
        # Add the scale label
        ax.text(-0.02 * sf1_total, 0, f'SF {scale}', va='center', ha='right', rotation=90, fontsize=fontsize,
                color='black')
...
def create_transfer_size_overhead_graph(csv_path, plot_pdf_path):
    ...
    # Optional: Customize the layout for better spacing and saving
...
def create_ranges_graph(json_path, plot_pdf):
    ...
    plt.yscale('log')
    # plt.xticks(df['number_of_ranges'].unique(),
    #            fontsize=12)  # Set x-ticks to only the unique values of number of ranges
...
def create_in_mem_convergence_graph(json_path, csv_path, plot_pdf):
    ...
    df_json_eager['real_time_no_transfer_no_scan'] = df_json_eager['real_time'] - (
                df_json_eager['number_of_queries'] * scan_time)
    ...
    # Original data - Lazy and Eager with solid lines
    plt.plot(df_json_lazy['number_of_queries'], df_json_lazy['real_time_no_transfer'], label='Lazy (w/o Transfer)',
             marker='*', linestyle=':',
             color='blue')
    plt.plot(df_json_eager['number_of_queries'], df_json_eager['real_time_no_transfer_no_scan'],
             label='Eager (w/o Transfer)',
             marker='P', linestyle=':',
             color='red')
...
def run_vtune_dads_overhead(benchmark_path, results_path, report_path, vtune_path):
...
def run_tpch_benchmark(benchmark_path, results_path):
    options = [f"--benchmark_out={results_path}", "--benchmark_out_format=json",
               "--benchmark_filter=BenchmarkQueriesTPCH"]
    run_executable(benchmark_path, options)


def run_tpch_parquet_benchmark(benchmark_path, results_path, filter):
    options = [f"--benchmark_out={results_path}", "--benchmark_out_format=json", str(filter)]
    run_executable(benchmark_path, options)
...
def create_averaged_tpch_parquet_json(final_result_path, result_dir_paths):
    sf_map = {
        0: "1",
        1: "10",
        2: "20"
    ...
    query_avg_res = {
        "1": {
            "parquet_avg_milliseconds": 0,
            "wisent_avg_milliseconds": 0,
            "parquet_bytes": 0,
            "wisent_bytes": 0
        },
        "10": {
    ...
    for result_dir_path in result_dir_paths:
        json_files = glob.glob(f'{result_dir_path}/*.json')
        csv_files = glob.glob(f'{result_dir_path}/*.csv')
        temp_res = calculate_averages_of_many_procs(json_files, csv_files)
        if "parquet" in result_dir_path:
            query_avg_res[sf_map[curr_sf]]["parquet_avg_milliseconds"] = temp_res["avg_milliseconds"]
            query_avg_res[sf_map[curr_sf]]["parquet_bytes"] = temp_res["avg_bytes"]
        else:
            query_avg_res[sf_map[curr_sf]]["wisent_avg_milliseconds"] = temp_res["avg_milliseconds"]
            query_avg_res[sf_map[curr_sf]]["wisent_bytes"] = temp_res["avg_bytes"]

        if curr_i > 0 and curr_i % 2 == 0:
            curr_sf += 1
            if curr_sf > 2:
                curr_sf = 0
                curr_query_res.append(query_avg_res.copy())

    final_res = {
        "queries": curr_query_res
    }

    with open(final_result_path, 'w') as final_json_file:
        json.dump(final_res, final_json_file, indent=4)


def calculate_averages_of_many_procs(json_files, csv_files):
    # Step 1: Calculate the average runtime from JSON files
    total_runtime = 0
    runtime_count = 0
    ...
    for json_file in json_files:
        with open(json_file, 'r') as f:
            ...
            for benchmark in benchmarks:
                total_runtime += benchmark['real_time']
                runtime_count += 1

    avg_milliseconds = total_runtime / runtime_count if runtime_count > 0 else 0

    # Step 2: Calculate the sum of Loaded, Requested, and Headers, then calculate the average
    total_sum = 0
    row_count = 0
    ...
    for csv_file in csv_files:
        if "size" in csv_file:
            with open(csv_file, 'r') as f:
                ...
                for row in reader:
                    total_sum += int(row['Loaded']) + int(row['Requested']) + int(row['Headers'])
                    row_count += 1

    avg_bytes = total_sum / row_count if row_count > 0 else 0
...
def handle_queries_benchmark(build_path):
    ...
    create_miniseed_queries_size_time_graph(str(results_path), str(size_results_path), str(time_results_path),
                                            str(output_pdf_path))
...
def handle_tpch_queries_parquet_benchmark(build_path):
    file_formats = [0, 1]
    queries = [0, 1, 2, 3, 4]
    sfs = [3, 4, 5]
    results_dir_paths = []
    for file_format in file_formats:
        for query in queries:
            for sf in sfs:
                results_dir_path = build_path.joinpath(f"results-{file_format}-{query}-{sf}")
                run_executable("mkdir", [str(results_dir_path)])
                results_dir_paths.append(str(results_dir_path))
                for i in range(6):
                    benchmark_path = build_path.joinpath("Benchmarks")
                    results_path = results_dir_path.joinpath(f"benchmark_tpch_results-{i}.json")
                    run_tpch_parquet_benchmark(str(benchmark_path), str(results_path),
                                               f"--benchmark_filter=BenchmarkQueriesTPCHParquet/{file_format}/{query}/{sf}/6/600000")

    final_result_path = build_path.joinpath("results", "tpch_parquet.json")
    create_averaged_tpch_parquet_json(final_result_path, results_dir_paths)
    output_pdf_path = build_path.joinpath("results", "plots", "benchmark_tpch_queries.pdf")
    create_tpch_parquet_graph(str(final_result_path), str(output_pdf_path))

    for file_format in file_formats:
        for query in queries:
            for sf in sfs:
                results_dir_path = build_path.joinpath(f"results-{file_format}-{query}-{sf}")
                if results_dir_path.is_dir():
...
def handle_framework_overhead_benchmark(build_path, vtune_path):
    ...
    run_vtune_dads_overhead(str(benchmark_path), str(results_path), str(report_path), vtune_path)
    create_dads_framework_overhead_graph(str(report_path), str(output_plot_pdf))
...
def main():
    parser = argparse.ArgumentParser(
        description="Run benchmarks and produce plots for DADSRemoteBinaryLoaderEngine when given the path to the build directory")
    ...
    handle_tpch_queries_parquet_benchmark(build_dir_path)
