...
#include "../Source/DADSLazyLoadingCoordinatorEngine.hpp"
#include <DADS.hpp>
...
std::unordered_map<std::string, std::function<dads::Expression()>> queryMap = {
    {"SIMPLE_SELECT_DADS",
     [] {
...
TEST_CASE("EvaluateInEnginesTest", "[]") { // NOLINT
  ...
  dads::engines::LazyLoadingCoordinator::Engine engine;
  auto delegatedEngineSymbols = ::std::vector<int64_t>();
  std::vector<std::string> enginePaths;
  enginePaths.push_back(
      "/home/david/Documents/PhD/symbol-store/DADSWisentDeserialiserEngine/"
      "debugBuild/libDADSWisentDeserialiserEngine.so");
  enginePaths.push_back(
      "/home/david/Documents/PhD/symbol-store/DADSConditionalEvaluationEngine/"
      "build/libDADSConditionalEvaluationEngine.so");
  enginePaths.push_back(RBL_PATH);
  enginePaths.push_back("/home/david/Documents/PhD/symbol-store/"
                        "DADSVeloxEngine/radixBuild/libDADSVeloxEngine.so");
  enginePaths.push_back(
      "/home/david/Documents/PhD/symbol-store/DADSDictionaryEncoderEngine/"
      "debugBuild/libDADSDictionaryEncoderEngine.so");
  ...
  auto newArgs = dads::ExpressionArguments();
  auto newSpanArgs = dads::expressions::ExpressionSpanArguments();
  newArgs.emplace_back(std::move("NonLECycle"_));
  newArgs.emplace_back(std::move(queryMap.at("RANGED_SELECT_DADS")()));
  newSpanArgs.emplace_back(
      dads::Span<int64_t>(::std::move(delegatedEngineSymbols)));
  auto *toEval = new DADSExpression{ComplexExpression(
      "EvaluateInEngines"_, {}, std::move(newArgs), std::move(newSpanArgs))};
  auto *r = new DADSExpression{engine.evaluate(std::move(toEval->delegate))};
  delete toEval;
