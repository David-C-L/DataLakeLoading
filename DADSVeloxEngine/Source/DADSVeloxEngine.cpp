#include "DADSVeloxEngine.hpp"

#include "DADSCoreTmp.h"
...
using dads::Span;
using dads::Symbol;
using dads::expressions::generic::isComplexExpression;
...
using ComplexExpression = dads::expressions::ComplexExpression;
template <typename... T>
using ComplexExpressionWithStaticArguments =
    dads::expressions::ComplexExpressionWithStaticArguments<T...>;
using Expression = dads::expressions::Expression;
using ExpressionArguments = dads::expressions::ExpressionArguments;
using ExpressionSpanArguments = dads::expressions::ExpressionSpanArguments;
...
template <typename T> dads::Span<T> createDadsSpan(VeloxVectorPayload&& payload) {
  auto* data = const_cast<T*>(payload.vec->values()->as<T>());
  auto length = payload.vec->size();
  return dads::Span<T>(data, length, [p = std::move(payload)]() {});
}

static ExpressionSpanArgument veloxtoSpan(VeloxVectorPayload&& payload) {
  switch(payload.vec->typeKind()) {
  case TypeKind::INTEGER:
    return createDadsSpan<int32_t>(std::move(payload));
  case TypeKind::BIGINT:
    return createDadsSpan<int64_t>(std::move(payload));
  case TypeKind::REAL:
    return createDadsSpan<float_t>(std::move(payload));
  case TypeKind::DOUBLE:
    return createDadsSpan<double_t>(std::move(payload));
  default:
...
template <typename T>
VectorPtr spanToVelox(dads::Span<T>&& span, memory::MemoryPool* pool, BufferPtr& indices) {
  ...
  auto createVeloxVector = [&indices, &createDictVector, &pool](auto&& span, auto dadsType) {
    auto spanLength = span.size();
    auto spanBegin = span.begin();
    DadsArray dadsArray(spanLength, spanBegin, std::move(span));
    auto flatVecPtr = importFromDadsAsOwner(dadsType, std::move(dadsArray), pool);
    if(indices == nullptr) {
      return flatVecPtr;
    }
  ...
  if constexpr(std::is_same_v<T, int8_t> || std::is_same_v<T, int8_t const>) {
    return createVeloxVector(std::move(span), DadsType::bCHAR);
  } else if constexpr(std::is_same_v<T, int32_t> || std::is_same_v<T, int32_t const>) {
    return createVeloxVector(std::move(span), DadsType::bINTEGER);
  } else if constexpr(std::is_same_v<T, int64_t> || std::is_same_v<T, int64_t const>) {
    return createVeloxVector(std::move(span), DadsType::bBIGINT);
  } else if constexpr(std::is_same_v<T, float_t> || std::is_same_v<T, float_t const>) {
    return createVeloxVector(std::move(span), DadsType::bREAL);
  } else if constexpr(std::is_same_v<T, double_t> || std::is_same_v<T, double_t const>) {
    return createVeloxVector(std::move(span), DadsType::bDOUBLE);
  }
}
...
static Expression dateProcess(Expression&& e) {
  return visit(dads::utilities::overload(
...
transformDynamicsToSpans(ComplexExpressionWithStaticArguments<StaticArgumentTypes...>&& input_) {
  std::vector<dads::SpanInputs> spanInputs;
  ...
  for(; it != std::move_iterator(dynamics.end()); ++it) {
    if(!std::visit(
           [&spanInputs]<typename InputType>(InputType&& argument) {
             ...
             if constexpr(dads::utilities::isVariantMember<std::vector<Type>,
                                                           dads::SpanInputs>::value) {
               if(!spanInputs.empty() &&
      ...
      std::back_inserter(spans), [](auto&& untypedInput) {
        return std::visit(
            []<typename Element>(std::vector<Element>&& input) -> ExpressionSpanArgument {
              ...
              return dads::Span<Element>(ptr, size, [ptr]() { spanReferenceCounter.remove(ptr); });
            },
...
static std::vector<BufferPtr> getIndices(ExpressionSpanArguments&& listSpans) {
  ...
  std::transform(std::make_move_iterator(listSpans.begin()),
                 ...
                 [](auto&& subSpan) {
                   return std::visit(
                       []<typename T>(dads::Span<T>&& typedSpan) -> BufferPtr {
                         if constexpr(std::is_same_v<T, int32_t>) {
                           auto spanLength = typedSpan.size();
                           auto spanBegin = typedSpan.begin();
                           DadsArray dadsIndices(spanLength, spanBegin, std::move(typedSpan));
                           return importFromDadsAsOwnerBuffer(std::move(dadsIndices));
                         } else {
...
getColumns(ComplexExpression&& expression, memory::MemoryPool* pool) {
  ...
  ExpressionArguments indicesColumnArgs;
  ...
  if(expression.getHead().getName() == "AddIndices") {
    auto [head, statics, dynamics, spans] = std::move(expression).decompose();
    auto it_start = std::move_iterator(dynamics.begin());
    expression = std::get<ComplexExpression>(std::move(*it_start));
    std::advance(it_start, 1);
    dads::Symbol indicesColumnHead = std::get<dads::Symbol>(std::move(*it_start));
    auto const& columns = expression.getArguments();

    if (columns.size() > 0) {
      auto const& firstColList = get<dads::ComplexExpression>(columns[0]).getArguments();
      auto const& firstList = get<dads::ComplexExpression>(firstColList[0]);
      auto const& firstListSpans = firstList.getSpanArguments();
      ExpressionSpanArguments indicesSpans;
      indicesSpans.reserve(firstListSpans.size());
      auto currIndex = 0;
      std::transform(firstListSpans.begin(), firstListSpans.end(), std::back_inserter(indicesSpans),
		     [&](auto const& subSpan) {
		       return std::visit(
					 [&](auto const& typedSubSpan) {
					   std::vector<int64_t> indicesVector;
					   indicesVector.reserve(typedSubSpan.size());
					   auto tempCurrIndex = currIndex;
					   for (auto i = 0; i < typedSubSpan.size(); currIndex++, i++) {
					     indicesVector.emplace_back(i + tempCurrIndex);
					   }
					   return std::move(dads::Span<int64_t>(std::move(indicesVector)));
					 },
					 std::forward<decltype(subSpan)>(subSpan));
		     });
      auto indicesListExpr = dads::ComplexExpression("List"_, {}, {}, std::move(indicesSpans));
      ExpressionArguments indicesArgs;
      indicesArgs.push_back(std::move(indicesListExpr));
      auto indicesColExpr = dads::ComplexExpression(std::move(indicesColumnHead), {}, std::move(indicesArgs), {});
      indicesColumnArgs.push_back(std::move(indicesColExpr));
    }
  }
  ...
  if(expression.getHead().getName() == "RadixPartition") {
    ...
    if(indicesSpans.size() == 1 && !tableDyn.empty()) {
      ...
      if(firstColumnSpans.size() > 1) {
        ...
        auto singleIndicesSpan = std::visit(
            []<typename T>(dads::Span<T>&& typedSpan) {
        ...
        for(auto& keyColumn : keyColumns) {
          ...
          list = dads::ComplexExpression(std::move(listHead), {}, std::move(listDynamics), {});
#ifdef USE_NEW_TABLE_FORMAT
...
#endif
          keyColumn =
              dads::ComplexExpression(std::move(head), {}, std::move(dynamics), std::move(spans));
        }
        // reconstruct the indices and the keys
        std::vector<ExpressionSpanArguments> newKeySpans(singleKeySpans.size());
        std::visit(
            [&]<typename T>(dads::Span<T>& typedIndicesSpan) {
              if constexpr(std::is_same_v<T, int32_t>) {
                ...
                auto reconstruct = [&]() {
                  ...
                  indicesSpans.emplace_back(
                      dads::Span<int32_t>(lastIt, length, [v = singleIndicesSpan]() {
                      }));
                  ...
                  for(; oldKeyIt != singleKeySpans.end(); ++oldKeyIt, ++newKeySpansIt) {
                    ...
                    std::visit(
                        [&]<typename U>(dads::Span<U> const& oldSingleKeyTypedSpan) {
                          newKeySpansIt->emplace_back(
                              dads::Span<U>(oldSingleKeyTypedSpan.begin() + offset, length,
                                            [v = oldSingleKeySpan]() {
                                            }));
                        },
                        *oldSingleKeySpan);
    ...
    // get table data
    expression = dads::ComplexExpression(std::move(tableHead), std::move(tableStatic),
  ...
  if (indicesColumnArgs.size() > 0) {
    // std::cout << "ADDING INDICES" << std::endl;
    std::transform(std::make_move_iterator(indicesColumnArgs.begin()),
		   std::make_move_iterator(indicesColumnArgs.end()), std::back_inserter(columns),
		   [](auto&& indicesColumnExpr) {
		     return std::move(indicesColumnExpr);
		   });
  }
  ...
  std::for_each(
      ...
      [&](auto&& columnExpr) {
        ...
        auto getVeloxColumnData = [&](auto listSpan, BufferPtr indices) {
          return std::visit(
              [pool, &indices, &columnName]<typename T>(dads::Span<T>&& typedSpan) -> VectorPtr {
                if constexpr(std::is_same_v<T, int8_t> || std::is_same_v<T, int32_t> ||
			     std::is_same_v<T, int64_t> || std::is_same_v<T, float_t> ||
			     std::is_same_v<T, double_t> || std::is_same_v<T, int8_t const> ||
			     std::is_same_v<T, int32_t const> ||std::is_same_v<T, int64_t const> ||
                             std::is_same_v<T, float_t const> || std::is_same_v<T, double_t const>) {
          ...
          for(auto spanIndex : otherColumnsSpanIndexes) {
            ...
            auto spanRef = std::visit(
                [&sharedSpan]<typename U>(
                    dads::Span<U> const& typedSpanRef) -> ExpressionSpanArgument {
                  return dads::Span<U>(typedSpanRef.begin(), typedSpanRef.size(),
                                       [v = sharedSpan]() {
                                       });
                },
                *sharedSpan);
...
PlanBuilder Engine::buildOperatorPipeline(
  ...
  if(e.getHead().getName() == "Table" || e.getHead().getName() == "Gather" ||
     e.getHead().getName() == "RadixPartition" || e.getHead().getName() == "AddIndices") {
    ...
    auto assignColumns = [](std::vector<std::string> names) {
      ...
      for(auto& name : names) {
        assignmentsMap.emplace(name, std::make_shared<DadsColumnHandle>(name));
      }
    ...
    auto plan = PlanBuilder(planNodeIdGenerator)
                    ...
                    .tableHandle(std::make_shared<DadsTableHandle>(
                        kDadsConnectorId, std::move(tableName), std::move(tableSchema),
...
dads::Expression Engine::evaluate(dads::Expression&& e) {
...
dads::Expression Engine::evaluate(dads::ComplexExpression&& e) {
  if(e.getHead().getName() == "ErrorWhenEvaluatingExpression") {
    return std::move(e);
  }
  if(e.getHead().getName() != "Group" && e.getHead().getName() != "Order" && e.getHead().getName() != "Top" &&
     e.getHead().getName() != "Project" && e.getHead().getName() != "Select" && e.getHead().getName() != "Join" &&
     e.getHead().getName() != "Set" && e.getHead().getName() != "GetEngineCapabilities") {
    auto [head, unused_, dynamics, spans] =
  ...
  if (e.getHead() == "GetEngineCapabilities"_) {
    return "List"_("Project"_, "Select"_, "Group"_, "Join"_, "Set"_, "Top"_, "Order"_);
  }
  if(e.getHead().getName() == "Group" || e.getHead().getName() == "Order" || e.getHead().getName() == "Top") {
    maxThreads = 2;
  } else {
  ...
  dads::expressions::ExpressionArguments columns;
...
#ifdef USE_NEW_TABLE_FORMAT
        dads::expressions::ExpressionArguments args;
...
Engine::Engine() {
  ...
  auto dadsConnector =
      connector::getConnectorFactory(dads::engines::velox::DadsConnectorFactory::kDadsConnectorName)
          ->newConnector(dads::engines::velox::kDadsConnectorId, nullptr);
  connector::registerConnector(dadsConnector);
}

Engine::~Engine() { connector::unregisterConnector(dads::engines::velox::kDadsConnectorId); }
...
static auto& enginePtr(bool initialise = true) {
  static std::mutex m;
  std::lock_guard const lock(m);
  static auto engine = std::unique_ptr<dads::engines::velox::Engine>();
  if(!engine && initialise) {
    engine.reset(new dads::engines::velox::Engine());
  }
  return engine;
}
