...
VectorPtr spanToVelox(boss::Span<T>&& span, memory::MemoryPool* pool, BufferPtr& indices) {
  ...
  if constexpr(std::is_same_v<T, int8_t> || std::is_same_v<T, int8_t const>) {
    return createVeloxVector(std::move(span), BossType::bCHAR);
...
getColumns(ComplexExpression&& expression, memory::MemoryPool* pool) {
  ...
  if(expression.getHead().getName() == "AddIndices") {
    ...
    boss::Symbol indicesColumnHead = std::get<boss::Symbol>(std::move(*it_start));
    auto const& columns = expression.getArguments();

    if (columns.size() > 0) {
      auto const& firstColList = get<boss::ComplexExpression>(columns[0]).getArguments();
      auto const& firstList = get<boss::ComplexExpression>(firstColList[0]);
		     ...
		     [&](auto const& subSpan) {
		       return std::visit(
					 [&](auto const& typedSubSpan) {
					   ...
					   return std::move(boss::Span<int64_t>(std::move(indicesVector)));
					 },
					 std::forward<decltype(subSpan)>(subSpan));
		     });
      auto indicesListExpr = boss::ComplexExpression("List"_, {}, {}, std::move(indicesSpans));
      ExpressionArguments indicesArgs;
      indicesArgs.push_back(std::move(indicesListExpr));
      auto indicesColExpr = boss::ComplexExpression(std::move(indicesColumnHead), {}, std::move(indicesArgs), {});
