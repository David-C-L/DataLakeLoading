...
#include "../Source/DADSRemoteBinaryLoaderEngine.hpp"
#include <DADS.hpp>
...
using dads::serialization::Argument;
using dads::serialization::ArgumentType;
using dads::serialization::RootExpression;
using dads::serialization::SerializedExpression;
using SerializationExpression = dads::serialization::Expression;
...
std::vector<int8_t> createByteBufferNamed(int32_t dataSetSize, int64_t sf) {
  ...
  RootExpression *root =
      SerializedExpression(
          "Table"_("value"_("List"_(dads::Span<int64_t>(vector(vec1))))))
          .extractRoot();
  ...
  // std::ofstream file("mediumLoaderNamedWisentTable.bin", std::ios::binary);
  // if (!file) {
...
std::vector<int8_t> createByteBuffer(int32_t dataSetSize, int64_t sf) {
  ...
  RootExpression *root =
      SerializedExpression("Table"_("List"_(dads::Span<int64_t>(vector(vec1)))))
          .extractRoot();
  size_t bufferSize = sizeof(Argument) * root->argumentCount +
                      sizeof(ArgumentType) * root->argumentCount +
                      sizeof(SerializationExpression) * root->expressionCount +
...
TEST_CASE("Allocate Whole File", "[basics]") { // NOLINT
  ...
  dads::engines::RBL::Engine engine;
  ...
  for (const auto &[sf, url] : sfUrlPairs) {
    ...
    auto expected =
        "ByteSequence"_(dads::Span<int8_t>(std::move(vector(bytes))));
    ...
    for (const auto &[sf, url] : sfUrlPairs) {
      ...
      auto [ununsed_1, unused_2, unused_3, spans] =
          std::move(std::get<ComplexExpression>(
              ...
              .decompose();
      auto &resultSpan = std::get<dads::Span<int8_t>>(spans.at(0));
      auto expectedSpan = dads::Span<int8_t>(std::move(vector(bytes)));
  ...
  for (const auto &[sf, url] : sfUrlPairs) {
    ...
    for (const auto &[lb, ub] : boundPairs) {
      for (size_t i = lb; i < ub; i++) {
        inBoundsEqual &= resultSpan.at(i) == expectedSpan.at(i);
        if (!inBoundsEqual) {
          std::cout << "FAILING AT: " << i << std::endl;
          break;
        }
      }
    }
...
TEST_CASE("156KB File Test With Padding & Alignment & Multipart & Span bounds",
  ...
  for (const auto &[sf, url] : sfUrlPairs) {
    ...
    auto [ununsed_1, unused_2, unused_3, spans] =
        std::move(
            std::get<ComplexExpression>(engine.evaluate("Fetch"_(
                "List"_(dads::Span<int64_t>(std::move(vector(boundPairsSpan)))),
                url, boundPadding, alignment, maxRanges))))
            .decompose();
    auto &resultSpan = std::get<dads::Span<int8_t>>(spans.at(0));
    auto expectedSpan = dads::Span<int8_t>(std::move(vector(bytes)));
...
TEST_CASE("Multipart & Singlepart",
          "[basics]") { // NOLINT
  auto const dataSetSize = 10000;
  dads::engines::RBL::Engine multiEngine;
  dads::engines::RBL::Engine singleEngine;
  std::vector<std::pair<int64_t, std::string>> sfUrlPairs = {
      {1, "https://www.doc.ic.ac.uk/~dcl19/largeWisentTable.bin"}};
  std::vector<std::pair<int64_t, int64_t>> boundPairs = {
  ...
  for (const auto &[sf, url] : sfUrlPairs) {
    ...
    auto [singleUnunsed_1, singleUnused_2, singleUnused_3, singleSpans] =
        std::move(std::get<ComplexExpression>(singleEngine.evaluate("Fetch"_(
                      ...
                      url, boundPadding, alignment))))
            .decompose();
    auto &multiResultSpan = std::get<dads::Span<int8_t>>(multiSpans.at(0));
    auto &singleResultSpan = std::get<dads::Span<int8_t>>(singleSpans.at(0));
    auto expectedSpan = dads::Span<int8_t>(std::move(vector(bytes)));
...
TEST_CASE("Allocate Multiple Whole Files", "[basics]") { // NOLINT
  ...
  auto expected = "ByteSequences"_(
      "ByteSequence"_(dads::Span<int8_t>(std::move(vector(bytes1)))),
      "ByteSequence"_(dads::Span<int8_t>(std::move(vector(bytes2)))));
